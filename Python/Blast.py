...
def blast_extract(frag_path, genome_path):
    ...
    q = check_output("echo Finished Blast", shell="True").decode('ascii')
    print q
    ...
    for hit in p.split('\n'):
        ...
        if len(value) > 1 and value[0] != last_hit:
            f.write(hit)
            f2.write(('Candidate %s found in %s between nt %s and %s\n' %(value[0], value[1], value[8], value[9])))
            last_hit = value[0]
